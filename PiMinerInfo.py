...
class PiMinerInfo:
	...
	screen6         = ['no data','no data']
	screen7         = ['no data','no data']
	...
	def __init__(self):
	  ...
	  self.refresh()
	  self.checkPrice()
	  #self.screen6[0] = 'HN:' + socket.gethostname()
	  #self.screen6[1] = 'IP:' + self.ipaddr
	...
	def hashrate(self, h):
	  u = 'Mh/s'
	  #Check hashrate for alert
	  if h < config.HASHRATE_MIN: self.n.sendAlert(0)
	  if h >= 1000.0:
		u = 'Gh/s'
		h = h / 1000.0
	  elif h >= 1000000.0:
	...
	def refresh(self):
		...
		self.screen6[0] = T.getTemp(0) + 'F'
		self.screen6[1] = T.getTemp(1) + 'C'
		self.checkTemp()

		
	def getTemp(self, temp):
                if(config.TEMP_ENABLED):
                        temperatures = self.sensor.get_temperatures([DS18B20.DEGREES_C, DS18B20.DEGREES_F, DS18B20.KELVIN])
